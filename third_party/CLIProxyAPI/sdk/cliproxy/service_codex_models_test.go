...
func TestRegisterModelsForAuthCodexAPIKeyModels(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantPresent: []string{"gpt-image-1.5", "gpt-image-2", "gpt-image-2.5"},
		},
		{
			...
			wantAbsent: []string{"gpt-image-1.5", "gpt-image-2", "gpt-image-2.5"},
		},
		{
...
func TestRegisterConfigAPIKeyAuthsCodexModelModes(t *testing.T) {
	...
	for index := range tests {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			for _, modelID := range []string{"gpt-image-1.5", "gpt-image-2", "gpt-image-2.5"} {
