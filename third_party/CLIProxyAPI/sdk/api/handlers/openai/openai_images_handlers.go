...
const (
	...
	cchImagesToolModel          = "cch-gpt-image-2"
	cchImages25ToolModel        = "cch-gpt-image-2.5"
...
func isCodexImagesToolModel(model string) bool {
	...
	return baseModel == gptImage15Model || baseModel == defaultImagesToolModel || baseModel == gptImage25Model || baseModel == cchImagesToolModel || baseModel == cchImages25ToolModel
}
...
func rejectUnsupportedImagesModel(c *gin.Context, model string) bool {
	...
	c.JSON(http.StatusBadRequest, handlers.ErrorResponse{
		Error: handlers.ErrorDetail{
			Message: fmt.Sprintf("Model %s is not supported on %s or %s. Use %s, %s, %s, %s, %s, %s, or a configured openai-compatibility image model.", model, imagesGenerationsPath, imagesEditsPath, gptImage15Model, defaultImagesToolModel, gptImage25Model, defaultXAIImagesModel, xaiImagesQualityModel, xaiImages20Model),
