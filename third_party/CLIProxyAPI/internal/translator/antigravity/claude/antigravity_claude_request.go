...
func ConvertClaudeRequestToAntigravity(modelName string, inputRawJSON []byte, _ bool) []byte {
	// Claude Code always sends Anthropic thinking metadata, even for Gemini
	// models that do not support thinking. Do not turn that metadata into a
	// Gemini thinkingConfig for the non-thinking Gemini 3 Flash family.
	enableThoughtTranslate := !isGeminiNoThinkingModel(modelName)
...
func isGeminiNoThinkingModel(modelName string) bool {
	switch strings.ToLower(strings.TrimSpace(modelName)) {
	case "gemini-3-flash", "gemini-3-flash-preview", "gemini-3-flash-agent":
		return true
	default:
		return false
	}
