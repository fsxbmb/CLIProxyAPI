...
func TestConvertClaudeRequestToAntigravity_DisablesThinkingForGemini3Flash(t *testing.T) {
	inputJSON := []byte(`{
	...
	for _, model := range []string{"gemini-3-flash", "gemini-3-flash-preview", "gemini-3-flash-agent"} {
		output := ConvertClaudeRequestToAntigravity(model, inputJSON, false)
		if gjson.GetBytes(output, "request.generationConfig.thinkingConfig").Exists() {
			t.Fatalf("%s unexpectedly contains thinkingConfig: %s", model, output)
		}
	}
}

func TestConvertClaudeRequestToAntigravity_PreservesThinkingForGeminiThinkingModel(t *testing.T) {
	inputJSON := []byte(`{
		"thinking":{"type":"adaptive"},
	...
	output := ConvertClaudeRequestToAntigravity("gemini-3.6-flash-high", inputJSON, false)
	if got := gjson.GetBytes(output, "request.generationConfig.thinkingConfig.thinkingLevel").String(); got != "high" {
		t.Fatalf("thinking level = %q, want high. Output: %s", got, output)
	}
}
